...
stock pointscap_save_config() {
    ...
    if (!file) {
        log_amx("[PointScap Editor] 无法写入配置文件: %s", filePath);
        return;
    }

    fprintf(file, "; ============================================^n");
    fprintf(file, "; PointScap Zone Configuration^n");
    fprintf(file, "; Auto-generated by PointScap Editor^n");
    fprintf(file, "; Map: %s^n", szMapName);
    fprintf(file, "; ============================================^n^n");

    for (new i = 0; i < g_iZoneCount; i++) {
        fprintf(file, "[%s]^n", g_szZoneLabels[g_eZones[i][ZONE_LABEL]]);

        // Write type
        fprintf(file, "type %d^n", g_eZones[i][ZONE_TYPE]);

        // Write generic mins/maxs (new format for pointscap_load_zones)
        fprintf(file, "mins %.0f %.0f %.0f^n",
            g_eZones[i][ZONE_MINS][0], g_eZones[i][ZONE_MINS][1], g_eZones[i][ZONE_MINS][2]);
        fprintf(file, "maxs %.0f %.0f %.0f^n",
            g_eZones[i][ZONE_MAXS][0], g_eZones[i][ZONE_MAXS][1], g_eZones[i][ZONE_MAXS][2]);

        // Also write point-type-specific format for backward compatibility
        // with pointscap_load_zones() which reads point3_mins/maxs etc.
        switch (g_eZones[i][ZONE_TYPE]) {
            case 3: {
                fprintf(file, "point3_mins %.0f %.0f %.0f^n",
                    g_eZones[i][ZONE_MINS][0], g_eZones[i][ZONE_MINS][1], g_eZones[i][ZONE_MINS][2]);
                fprintf(file, "point3_maxs %.0f %.0f %.0f^n",
                    g_eZones[i][ZONE_MAXS][0], g_eZones[i][ZONE_MAXS][1], g_eZones[i][ZONE_MAXS][2]);
                break;
            }
            case 4: {
                fprintf(file, "point4_mins %.0f %.0f %.0f^n",
                    g_eZones[i][ZONE_MINS][0], g_eZones[i][ZONE_MINS][1], g_eZones[i][ZONE_MINS][2]);
                fprintf(file, "point4_maxs %.0f %.0f %.0f^n",
                    g_eZones[i][ZONE_MAXS][0], g_eZones[i][ZONE_MAXS][1], g_eZones[i][ZONE_MAXS][2]);
                break;
            }
            case 5: {
                fprintf(file, "point5_mins %.0f %.0f %.0f^n",
                    g_eZones[i][ZONE_MINS][0], g_eZones[i][ZONE_MINS][1], g_eZones[i][ZONE_MINS][2]);
                fprintf(file, "point5_maxs %.0f %.0f %.0f^n",
                    g_eZones[i][ZONE_MAXS][0], g_eZones[i][ZONE_MAXS][1], g_eZones[i][ZONE_MAXS][2]);
                break;
            }
        }
    ...
    log_amx("[PointScap Editor] 地图配置已保存: %s (%d 个点位)", filePath, g_iZoneCount);
...
stock bool:pointscap_editor_append_zone(labelIdx, typeVal, Float:fMins[3], Float:fMaxs[3]) {
    ...
    switch (typeVal) {
        case 4: g_eZones[idx][ZONE_SCORE] = 1.0;
        case 5: g_eZones[idx][ZONE_SCORE] = 2.0;
        default: g_eZones[idx][ZONE_SCORE] = 0.5;
    }
...
stock pointscap_load_config() {
    ...
    while (!feof(file) && g_iZoneCount < MAX_ZONES) {
        ...
        if (line[0] == '[') {
            if (currentLabel >= 0) {
                if (hasGenericMins && hasGenericMaxs) {
                ...
                } else if (explicitType >= 3 && explicitType <= 5 &&
    ...
    log_amx("[PointScap Editor] 配置已加载: %s (%d 个点位)", filePath, g_iZoneCount);
}
